...
export default function DossierWorkflow() {
  ...
  const handleSaveDossier = async () => {
    ...
    setIsSaving(true);
    try {
      // Build the payload using the exact backend DOCUMENT_STATUS_CHOICES keys:
      // ('yes', 'Yes / Available'), ('na', 'N/A'), ('missing', 'Missing / Absent')
      const docPayload: Record<string, string | null> = {};
      for (const doc of DOSSIER_DOCS) {
        const apiField = DOC_TO_API[doc];
        if (!apiField) continue;
        const raw = selectedEmployee.documents[doc];
        if (raw === 'present') docPayload[apiField] = 'yes';
        else if (raw === 'missing') docPayload[apiField] = 'missing';
        else if (raw === 'n/a') docPayload[apiField] = 'na';
      }
